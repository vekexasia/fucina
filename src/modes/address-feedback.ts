...
export async function addressFeedback(event: FucinaEvent, deps: { gh(args: string[]): string; agent(prompt: string): Promise<AgentResult> }) {
  ...
  deps.gh(["pr", "comment", String(event.number), "--body", String(parseFucinaJson(result.stdout).summary)]);
}
