...
export async function review(event: FucinaEvent, deps: { gh(args: string[]): string; agent(prompt: string): Promise<AgentResult> }) {
  ...
  deps.gh(["pr", "review", String(event.number), "--comment", "--body", String(result.summary)]);
}
