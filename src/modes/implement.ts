...
export async function implement(event: FucinaEvent, deps: { gh(args: string[]): string; agent(prompt: string): Promise<AgentResult>; sh?(args: string[]): void; runUrl?: string }) {
  ...
  const sh = deps.sh ?? ((args: string[]) => execFileSync(args[0], args.slice(1), { stdio: "inherit" }));
