...
export async function runEvent(event: FucinaEvent, deps: Deps) {
  ...
  try {
    ...
    if (event.label === "fucina:explore" && event.kind === "issue") return await explore(event, modeDeps);
    if (event.label === "fucina:implement" && event.kind === "issue") return await implement(event, modeDeps);
    if (event.label === "fucina:address-feedback" && event.kind === "pull_request") return await addressFeedback(event, modeDeps);
    if (event.label === "fucina:review" && event.kind === "pull_request") return await review(event, modeDeps);
